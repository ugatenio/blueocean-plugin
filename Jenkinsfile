        ...
        docker.image('blueocean_build_env').inside("--net=container:blueo-selenium") {
          ...
          stage('Building BlueOcean') {
            timeout(time: 90, unit: 'MINUTES') {
              sh "mvn clean install -V -B -DcleanNode -B -Dmaven.test.failure.ignore -s settings.xml -Dmaven.artifact.threads=30 -Dmaven.repo.local=${env.WORKSPACE}/.m2/repository"
            }
            ...
            stage("ATH - Jenkins ${version}") {
              timeout(time: 90, unit: 'MINUTES') {
                dir('acceptance-tests') {
                  sh "bash -x ./run.sh -v=${version} --host=${ip} --no-selenium --settings='-s ${env.WORKSPACE}/settings.xml -Dmaven.repo.local=${env.WORKSPACE}/.m2/repository'"
